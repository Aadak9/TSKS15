...
n = -100:100;         
s1_sampled = exp(-0.1 * (n*Ts).^2);
s2_sampled = exp(-0.1 * (n*Ts).^2) .* cos(n*Ts);
...
%ML estimation of T
%{
ML_estimation_vector = [];
for  T = -5:Ts:5
    sweep_signal = (exp( -0.1* (Trange-T) .^2) .*cos(Trange-T)).*x;
    ML_estimation_vector(T) = sum(sweep_signal);
end
[~, idx] = max(ML_estimation_vector);
T_hat = T(idx)
%}
