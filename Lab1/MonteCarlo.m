...
%computing time
error_probabilities = zeros(size(SNR_db)); %Will contain the error-probability for each SNR
for i = 1:length(SNR_db)
    ...
    error_probabilities(i) = missclassification_error/runs;
end
size(SNR_db)
...
%{
SNR_db = -50:2:-10;
error_probabilities = zeros(size(SNR_db));
for i = 1:length(SNR_db)
    snr_val = SNR_db(i);
    missclassification_error = 0;
    runs = 0;
...
xlabel('SNR [dB]');
ylabel('Probability of error');
title('ThreeToneclassifier with 3 tone');
grid on;
%}